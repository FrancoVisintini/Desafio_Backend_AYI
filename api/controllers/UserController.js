...
const signUpUserController = async (req, res) => {
    ...
    const userAlreadyExists = await Users.find({email})
    if(userAlreadyExists.length!==0) return res.send('User already exists. Please login or create a different user');
...
const editUserController = async (req, res) => {
    ...
    try {
        ...
        await user.update({ $set: { first_name, last_name, email } })
